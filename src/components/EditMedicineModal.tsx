...
interface EditMedicineModalProps {
    isOpen: boolean;
    onClose: () => void;
    onUpdate: (id: string, medicine: MedicineFormData) => void;
    medicine: any | null;
}
...
export default function EditMedicineModal({ isOpen, onClose, onUpdate, medicine }: EditMedicineModalProps) {
    ...
    useEffect(() => {
        if (medicine) {
            setFormData({
                ...
                expiry: medicine.expiry ? new Date(medicine.expiry).toISOString().split('T')[0] : '',
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (medicine) {
            onUpdate(medicine._id, formData);
        }
        handleClose();
    };
            ...
            <div className="bg-white rounded-2xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-hidden">
                ...
                <div className="bg-gradient-to-r from-[#2EBE76] to-[#0BAF8C] px-6 py-5 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-2xl font-bold text-white">Edit Medicine</h2>
                            <p className="text-white text-opacity-90 text-sm font-secondary">Update medicine details</p>
                        </div>
                    </div>
                    <button
                        onClick={handleClose}
                ...
                <form onSubmit={handleSubmit} className="p-6 overflow-y-auto max-h-[calc(90vh-100px)]">
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    <input
                                        ...
                                        className="w-full px-4 py-3 bg-[#F7FDFC] border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#2EBE76] focus:border-transparent transition-all"
                                    />
                                    <Calendar className="absolute right-4 top-1/2 transform -translate-y-1/2 text-[#6C757D] w-5 h-5 pointer-events-none" />
                                </div>
                            </div>
                    ...
                    <div className="flex gap-4 mt-8 pt-6 border-t border-gray-100">
                        <button
                            type="button"
                            onClick={handleClose}
                            className="flex-1 px-6 py-3 border border-gray-200 rounded-xl font-semibold text-[#1A1A1A] hover:bg-gray-50 transition-all"
                        >
                            Cancel
                        </button>
                        <button
